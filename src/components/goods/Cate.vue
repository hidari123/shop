    ...
    <el-card>
      ...
      <zk-table class="treeTable" :data="cateList" :columns="columns" :selection-type="false" :expand-type="false"
        ...
        <template v-slot:order="scoped">
          <el-tag type="primary" size="mini" v-if="scoped.row.cat_level === 0">一级</el-tag>
          <el-tag type="success" size="mini" v-else-if="scoped.row.cat_level === 1">二级</el-tag>
          <el-tag type="warning" size="mini" v-else>三级</el-tag>
        </template>
        <!--        操作-->
        <template v-slot:opt="scoped">
          <el-button type="primary" @click="showEditCateDialog(scoped.row.cat_id)" icon="el-icon-edit" size="mini">编辑
          </el-button>
          <el-button type="danger" @click="removeCateById(scoped.row.cat_id)" icon="el-icon-delete" size="mini">删除
          </el-button>
        </template>
      </zk-table>
      <!--    分页区域-->
...
export default {
  ...
  methods: {
    ...
    // 选择项发生变化 触发这个函数
    parentCateChanged () {
      // 如果 selectKeys 数组中的 length 大于0 证明选中的父级分类
      // 反之 就说明没有选中任何父级分类
